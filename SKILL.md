...
2. **Text baked into images.** Any headline/label/CTA is generated INTO the art by the image model, with the exact words in quotes in the prompt. NEVER add text in post — no overlays, no re-typesetting, no drawtext. Final assembly must use the approved keyframes EXACTLY as generated. Text only on beats that need it (hook, key number, CTA); leave in-between shots clean. Verify spelling on every returned frame before animating.
...
3. **Animate** — image-to-video per rule 5, ~5s per clip, subtle motion. EVERY scene must be a true image-to-video clip: never loop, shake, wobble, or freeze-hold a still to fill time. Poll until all complete.
...
5. **Assemble (ffmpeg)** — per scene: scale/crop to 720×1280, stretch the clip smoothly to that line's duration (`setpts` — slow it, never freeze it), 30fps; concat all scenes; mux VO + music (`amix`, `loudnorm I=-14`); apply the speed-up if needed. The first VO word lands at frame zero — no music-only intro. The final CTA scene holds no more than 3 seconds and keeps gentle motion to the end.
